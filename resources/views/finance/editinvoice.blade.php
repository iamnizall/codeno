...
<div class="container">
	<div class="card">
		...
		<div class="card-body">
			<form method="POST" action="{{ route('invoice.update', $invc->id) }}">
				@csrf
				@method('PUT')
				{{-- row 1 --}}
				<div class="form-row">
				...
				<div class="form-row mt-4">
					...
					<div class="col">
						...
						<select name="tax" id="tax" class="form-control" required value="{{ $invc->tax }}">
							<option>Tax</option>
							<option value="0.12" @if ($invc->tax == '0.12')
								selected 
								@endif>PPN 10%</option>
								<option value="0.05" @if ($invc->tax == '0.05')
									selected 
									@endif>PPh 21 (freelancer NPWP)</option>
									<option value="0.06" @if ($invc->tax == '0.06')
										selected 
										@endif>PPh 21 (freelancer non NPWP)</option>
										<option value="0.025"  @if ($invc->tax == '0.025')
											selected 
											@endif>PPh 21 (expert NPWP)</option>
											<option value="0.03" @if ($invc->tax == '0.03')
												selected 
												@endif>PPh 21 (expert non NPWP)</option>
												<option value="0.02" @if ($invc->tax == '0.02')
													selected 
													@endif>PPh 23 (vendor)</option>
												</select>
											</div>
										</div>
										...
										<div class="form-row mt-4">
											...
											<div class="col">
												...
												<select id="norek" name="norek" class="form-control" required>
													...
													<option value="070 1137302" @if ($invc->norek == '070 1137302')
														selected 
														@endif>IDR</option>
														<option value="090 2211411" @if ($invc->norek == '090 2211411')
															selected 
															@endif>Dollar</option>
															<option value="090 2212221" @if ($invc->norek == '090 2212221')
																selected 
																@endif>Euro</option>
																<option value="3590119073" @if ($invc->norek == '3590119073')
																	selected 
																	@endif>IDR(Danamon Bank)</option>
																	<option value="financedept@bintang‐35.net" @if ($invc->norek == 'financedept@bintang‐35.net')
																		selected 
																		@endif>Paypal</option>							
																	</select>
																</div>
